...
class TrainDataset():
    def __init__(self, data_path, model,tokenizer):
           ...
           self.type_of_graph = ["ARGUMENT", "RECEIVER", "CALL", "REACHING_DEF", "CDG", "CFG", "AST"]
    ...
    def get_graph_and_testcase(self, index):
    ...
    def get_node_features(self, graph):
        ...
        for code in df["CODE"].tolist():
            inputs = self.tokenizer(code, padding=True, truncation=True, return_tensors="pt", max_length=128).to(self.model.device)
            with torch.no_grad():
                embedding = self.model.encoder(**inputs).last_hidden_state.mean(dim=1)[0]
    ...
    def read_edge(self, graph):
        ...
        for edge in graph['edges']:
            if edge['label'] not in edge_dict:
                edge_dict[edge['label']] = [[node_dict[edge['src']]],[node_dict[edge['dst']]]]
            else:
...
def main():
    ...
    data.preprocess_pipeline(args.dataset_file, args.graph_file, args.mask_file, tokenizer_Qwen)
